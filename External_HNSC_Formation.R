...
## ============================================================
## Process HNSC tumor and normal files
## ============================================================

hnsc_tumor <- process_cptac_file(
  expr_file = "HNSCC_RNAseq_gene_RSEM_coding_UQ_1500_log2_Tumor.txt",
  class_name = "hnsc"
)

hnsc_normal <- process_cptac_file(
  expr_file = "HNSCC_RNAseq_gene_RSEM_coding_UQ_1500_log2_Normal.txt",
  class_name = "normal"
)
...
common_genes <- intersect(
  colnames(hnsc_tumor),
  colnames(hnsc_normal)
)

common_genes <- setdiff(common_genes, "Class")

hnsc_tumor_common <- hnsc_tumor[, c(common_genes, "Class")]
hnsc_normal_common <- hnsc_normal[, c(common_genes, "Class")]
...
cat("\nFinal HNSC + Normal dimensions:\n")
print(dim(hnsc_final))

cat("\nFinal class distribution:\n")
print(table(hnsc_final$Class))
...
write.csv(
  hnsc_final,
  "CPTAC_HNSC_NORMAL.csv",
  row.names = FALSE
)

cat("\nSaved file: CPTAC_HNSC_NORMAL.csv\n")