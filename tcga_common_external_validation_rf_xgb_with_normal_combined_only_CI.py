# ============================================================
# TCGA multiclass classification with Normal class
# Per-dataset StandardScaler normalization
# Models: Random Forest, XGBoost
# Hyperparameter optimization: Bayesian Optimization
# Ensemble: weighted soft voting
# Validation strategy:
#   1) TCGA is standardized before train/test split.
#   2) Each external validation dataset is standardized separately.
#   3) Standardized validation datasets are then concatenated.
#   4) Metrics are reported only for TCGA train/test and combined validation.
#   5) Classification metrics include 95% bootstrap confidence intervals.
# ============================================================
...
# Training dataset WITH Normal class
...
OUT_DIR = "Classification_Results_RF_XGB_with_normal_per_dataset_scaled_combined_validation_CI"
...
def standardize_expression_profiles(X, dataset_name):
    """Standardize gene-expression columns independently for one dataset."""
...
def bootstrap_confidence_intervals(y_true, y_pred, n_bootstrap=BOOTSTRAP_ITERATIONS,
    ...
    metric_values = {
        ...
        "F1_macro": [], "Precision_weighted": [], "Recall_weighted": [], "F1_weighted": [], "MCC": [],
    }
    if n == 0:
...
def calculate_metrics_with_ci(y_true, y_pred, dataset_name, model_name):
    metrics = calculate_metrics(y_true, y_pred, dataset_name, model_name)
    metrics.update(bootstrap_confidence_intervals(y_true, y_pred))
    return metrics
...
def save_report_and_cm(y_true, y_pred, target_names, model_name, dataset_name, out_dir):
    ...
    cm_df = pd.DataFrame(cm, index=[f"True_{cls}" for cls in target_names], columns=[f"Pred_{cls}" for cls in target_names])
...
def save_predictions(y_true, y_pred, y_prob, original_indices, target_names, label_encoder,
                     model_name, dataset_name, out_dir, validation_dataset=None):
    ...
    if validation_dataset is not None:
        pred_df["Validation_dataset"] = validation_dataset
...
def evaluate_model_on_dataset(model, model_name, X_eval, y_eval_enc, original_indices,
    ...
    metrics = calculate_metrics_with_ci(y_eval_enc, y_pred, dataset_name, model_name)
...
def build_scaled_combined_validation(validation_files, train_features):
    """Standardize each external validation dataset separately, then concatenate."""
    frames = []
    for dataset_name, file_path in validation_files.items():
        print(f"Preparing validation dataset for combined validation: {dataset_name}")
        val_data = read_dataset(file_path, CLASS_COL)
        print(f"{dataset_name} raw shape:", val_data.shape)
        print(f"{dataset_name} class distribution:")
        print(val_data[CLASS_COL].value_counts())
        X_val, y_val = split_xy(val_data, CLASS_COL)
        X_val = align_validation_features(X_val, train_features, dataset_name)
        X_val = standardize_expression_profiles(X_val, dataset_name)
        tmp = X_val.copy()
        tmp[CLASS_COL] = y_val.values
        tmp["Validation_dataset"] = dataset_name
        frames.append(tmp)
    combined_scaled = pd.concat(frames, axis=0, ignore_index=True)
    return combined_scaled

# ============================================================
# 3) Load TCGA data and standardize before train/test split
# ============================================================
...
train_idx, test_idx = train_test_split(sample_indices, test_size=TEST_SIZE, random_state=RANDOM_STATE, stratify=y)
...
for dataset_name, y_eval, idx in [("Train", y_train_enc, train_idx), ("Test", y_test_enc, test_idx)]:
    ...
    ens_metrics = calculate_metrics_with_ci(y_eval, pred, dataset_name, "WeightedSoftVoting")
...
# ============================================================
# 7) Combined external validation only
# ============================================================

combined_data_scaled = build_scaled_combined_validation(VALIDATION_FILES, feature_names)

print("Combined standardized validation shape:", combined_data_scaled.shape)
print("Combined standardized validation class distribution:")
print(combined_data_scaled[CLASS_COL].value_counts())

X_comb, y_comb = split_xy(combined_data_scaled, CLASS_COL)
...
validation_dataset_labels = combined_data_scaled["Validation_dataset"].values
validation_metrics = []
combined_prob_by_model = {}

for model_name, model in models.items():
    metrics, pred, prob = evaluate_model_on_dataset(
        model, model_name, X_comb, y_comb_enc, np.arange(combined_data_scaled.shape[0]),
        "Validation_ALL", target_names, label_encoder, OUT_DIR,
        validation_dataset=validation_dataset_labels,
    )
    validation_metrics.append(metrics)
    combined_prob_by_model[model_name] = prob

pred, prob = weighted_soft_vote(combined_prob_by_model, model_order, weights)
combined_ens_metrics = calculate_metrics_with_ci(y_comb_enc, pred, "Validation_ALL", "WeightedSoftVoting")
...
save_predictions(
    y_comb_enc, pred, prob, np.arange(combined_data_scaled.shape[0]),
    target_names, label_encoder, "WeightedSoftVoting", "validation_ALL", OUT_DIR,
    validation_dataset=validation_dataset_labels,
)

# ============================================================
# 8) Save metrics, feature importance, models, summary
# ============================================================
...
validation_metrics_df.to_csv(os.path.join(OUT_DIR, "combined_external_validation_classification_metrics.csv"), index=False)
...
best_test_model = all_metrics_df[all_metrics_df["Dataset"] == "Test"].sort_values(
    "F1_weighted", ascending=False
).iloc[0].to_dict()

best_combined_validation_model = all_metrics_df[all_metrics_df["Dataset"] == "Validation_ALL"].sort_values(
    "F1_weighted", ascending=False
).iloc[0].to_dict()
...
summary = {
    "dataset": DATA_PATH,
    "analysis_type": (
        "TCGA multiclass classification with Normal class; per-dataset StandardScaler normalization; "
        "external validation evaluated only on combined validation data"
    ),
    ...
    "validation_strategy": (
        "Each external validation dataset is standardized independently using StandardScaler, then all "
        "standardized validation datasets are concatenated into one combined validation set. Metrics are "
        "reported only for the combined validation set."
    ),
    "standardization": {
        "method": "sklearn.preprocessing.StandardScaler",
        "tcga_strategy": "TCGA expression matrix is standardized before train/test split.",
        "external_validation_strategy": "Each validation dataset is standardized independently before concatenation.",
        "class_column": "Excluded from standardization.",
    },
    ...
    "best_test_model_by_f1_weighted": best_test_model,
    "best_combined_validation_model_by_f1_weighted": best_combined_validation_model,
}
...
print(best_combined_validation_model)
