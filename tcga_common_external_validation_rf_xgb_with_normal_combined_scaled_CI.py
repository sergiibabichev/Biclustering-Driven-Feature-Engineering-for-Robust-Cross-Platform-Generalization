...
# Combined-validation StandardScaler normalization
# Models: Random Forest, XGBoost
# Hyperparameter optimization: Bayesian Optimization
# Ensemble: weighted soft voting
# Validation strategy:
#   1) TCGA is standardized before train/test split.
#   2) External validation datasets are first concatenated.
#   3) The complete combined validation matrix is standardized once.
...
OUT_DIR = "Classification_Results_RF_XGB_with_normal_combined_validation_scaled_CI"
...
def build_combined_validation(validation_files, train_features):
    """
    Concatenate external validation datasets first, then return one combined
    feature matrix that will be standardized once as a complete validation set.
    """
    frames = []
    ...
    for dataset_name, file_path in validation_files.items():
        ...
        X_val = align_validation_features(
            X_val,
            train_features,
            dataset_name,
        )
...
# ============================================================
# 7) Combined external validation only: concatenate first, then standardize
# ============================================================

combined_data = build_combined_validation(VALIDATION_FILES, feature_names)

print("Combined validation shape before standardization:", combined_data.shape)
print("Combined validation class distribution:")
...
# Standardize the complete combined validation matrix once.
# This is methodologically symmetric with TCGA scaling: all classes are scaled together.
...
validation_dataset_labels = combined_data["Validation_dataset"].values
validation_metrics = []
...
save_predictions(
    y_comb_enc, pred, prob, np.arange(combined_data.shape[0]),
...
summary = {
    "dataset": DATA_PATH,
    "analysis_type": (
        "TCGA multiclass classification with Normal class; combined-validation StandardScaler normalization; "
    ...
    "validation_strategy": (
        "External validation datasets are concatenated first. The complete combined "
        "validation matrix is then standardized once using StandardScaler. Metrics are "
    ...
    "standardization": {
        ...
        "external_validation_strategy": "External validation datasets are concatenated first and then standardized once as one combined validation matrix.",
